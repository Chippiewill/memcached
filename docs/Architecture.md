...
##### Memcached Binary Protocol (mcbp)
Couchbase smart clients presently connect using a protocol derived from the
Memcached binary protocol. The principle difference from the Memcached binary
protocol are the additional opcodes for Couchbase specific operations such as
DCP and subdoc. More in-depth explanation of the binary protocol used by
